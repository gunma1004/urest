...
const shopData: Record<string, {
  ...
  "1": {
    ...
    features: ["100% 안심 현장 결제", "세부 동 25분 내 도착", "24시간 상시 운영", "위생 및 방역 철저"]
  },
  "2": {
    name: "🌸 한국미인테라피",
  ...
  "4": {
    ...
    features: ["100% 현장 결제 안심 시스템", "전문 테라피스트 배정", "세부 동 전지역 방문", "24시간 예약 가능"]
  },
  "5": {
    name: "🌙 오늘밤테라피",
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const pageTitle = `${locationPrefix} 출장 타이 마사지 24시 안내 - ${shop.name} | 유레스트`;
...
export default async function DongShopDetailPage({ params }: PageProps) {
  ...
  const locationPrefix = parseLocationText(region, resolvedParams.district, resolvedParams.dong);
    ...
    <div className="bg-[#08080a] text-gray-100 min-h-screen flex flex-col font-sans selection:bg-amber-500 selection:text-black pb-28">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 w-full flex-1 space-y-8">
        ...
        <section className="bg-black/80 p-5 rounded-2xl border border-white/10">
          <h3 className="text-amber-400 font-bold text-sm mb-2 flex items-center gap-1.5">
            <span>📌</span> {locationPrefix} 이용 예약 안내
          </h3>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#08080a]/95 backdrop-blur-xl border-t border-amber-500/30 p-3 md:p-4 shadow-[0_-10px_25px_rgba(0,0,0,0.8)]">
        <div className="max-w-4xl mx-auto grid grid-cols-2 gap-3">
          ...
          <a 
            href={`sms:${shop.phone.replace(/-/g, "")}?body=${encodeURIComponent(`[${locationPrefix}] ${shop.name} 예약 문의드립니다.`)}`}
